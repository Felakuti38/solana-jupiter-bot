...
const swap = async (jupiter, route) => {
	try {
		...
		const priority =
			typeof cache.config.priority === "number" ? cache.config.priority : 100; //100 BPS default if not set
...
const failedSwapHandler = async (tradeEntry, inputToken, tradeAmount) => {
	...
	if (Number(realbalanceToken) < Number(tradeAmount)) {
		...
		if (cache.tradeCounter.failedbalancecheck > 5) {
			console.log(
				`Balance Lookup is too low for token: ${realbalanceToken} < ${tradeAmount}`
			);
	...
	if (cache.tradeCounter.errorcount > 100) {
		console.log(
			`Error Count is too high for swaps: ${cache.tradeCounter.errorcount}`
		);
		...
		try {
			...
			const fetcher = async (retry) => {
				...
				if (rcount >= retries) {
					// Exit max retries
					console.log(
						`Reached max attempts to fetch transaction. Assuming it did not complete.`
					);
					return -1;
				}
				...
				if (err2 == 0 && txresult) {
					...
					console.log("Transaction failed:", JSON.stringify(txresult));
					return -1;
				} else {
					retry(new Error("Transaction was not posted yet. Retrying..."));
				}
