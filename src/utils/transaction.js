...
const checktrans = async (txid, wallet_address) => {
	try {
		...
		if (transresp) {
			...
			if (
				!transresp.meta.postTokenBalances ||
				transresp.meta.postTokenBalances.length === 0
			) {
			...
			for (const token of transresp.meta.postTokenBalances) {
				if (token.owner == wallet_address) {
					if (transaction_changes[token.mint]?.start) {
						...
						const diff =
							Number(token.uiTokenAmount.amount) -
							Number(transaction_changes[token.mint].start);
						const diffdec = toDecimal(
							diff,
							transaction_changes[token.mint].decimals
						);
