...
const validateEnvironment = () => {
	const required = ["SOLANA_WALLET_PRIVATE_KEY", "DEFAULT_RPC"];
...
const validateSlippage = (slippage) => {
	...
	if (slippage < 0 || slippage > TRADING_CONSTANTS.MAX_SLIPPAGE) {
		throw new Error(
			`Slippage must be between 0 and ${TRADING_CONSTANTS.MAX_SLIPPAGE}`
		);
	}
...
const validateTradeSize = (size, balance) => {
	...
	if (size < balance * TRADING_CONSTANTS.MIN_POSITION_SIZE) {
		throw new Error(
			`Trade size must be at least ${
				TRADING_CONSTANTS.MIN_POSITION_SIZE * 100
			}% of balance`
		);
	}
	...
	if (size > balance * TRADING_CONSTANTS.MAX_POSITION_SIZE) {
		throw new Error(
			`Trade size cannot exceed ${
				TRADING_CONSTANTS.MAX_POSITION_SIZE * 100
			}% of balance`
		);
	}
...
const validateConfig = (config) => {
	...
	if (
		!config.network ||
		!["mainnet", "devnet", "testnet"].includes(config.network)
	) {
	...
	if (
		...
		!["pingpong", "arbitrage"].includes(config.tradingStrategy)
	) {
	...
	if (
		config.tradingStrategy === "pingpong" &&
		(!config.tokenB || !config.tokenB.address || !config.tokenB.symbol)
	) {
	...
	if (
		!config.tradeSize ||
		typeof config.tradeSize.value !== "number" ||
		config.tradeSize.value <= 0
	) {
...
const validateSafetyLevel = (level) => {
	...
	if (!validLevels.includes(level)) {
		throw new Error(
			`Invalid safety level. Must be one of: ${validLevels.join(", ")}`
		);
	}
...
const validateAMMStrategy = (strategy) => {
	...
	if (!validStrategies.includes(strategy)) {
		throw new Error(
			`Invalid AMM strategy. Must be one of: ${validStrategies.join(", ")}`
		);
	}
